...
//void setup() {
 // pinMode(ledPin, OUTPUT);
  ...
  //Serial.begin(9600);
 //SWITCHCAPVCC, 0x3C);  // initialize with the I2C addr 0x3C (for the 128x64)
  //display.clearDisplay();

  //digitalWrite(ledPin, HIGH);                 // Status led on...
//}
//NOTE ALL THE CHANGES FOR DAISY
 ...
 /** This method generates a range based on param.Init() values **/
  ...
  if (gameState == 0) {
   ...
   // display.println("PONG");
    display.WriteString("PONG ", Font_6x8, true);
    //display.setTextSize(1);
    //display.setTextColor(WHITE);
   // display.setCursor(27, 20);
        display.SetCursor(27, 20);
   // display.println("press start");
        display.WriteString("press start ", Font_7x10, true);
    //display.display();
    //display.clearDisplay();    
    display.Update();  
    display.Fill(false);
  ...
  if (gameState == 2) {
   ...
   // display.println("winner!");
        display.WriteString("Winner!!!", Font_6x8, true);
   // display.display();
    //display.clearDisplay();   
    display.Update();  
    display.Fill(false);
...
display.WriteString(score1Str.c_str(), Font_11x18, true);
    display.WriteString(":", Font_6x8, true);
    std::string score2Str = std::to_string(score1);
display.WriteString(score2Str.c_str(), Font_11x18, true);
...
fillRectangle(63, 22, 1, 5, 1);
  ...
  fillRectangle(63, 52, 1, 5, 1);
   fillRectangle(63, 62, 1, 5, 1);
    // display.DrawRect(0,128, 5, 1, true, false);  
     //display.DrawRect(5,128, 5, 1, true, false);  
     //display.DrawRect(35,128, 5, 1, true, false);  
     //display.DrawRect(60,128, 5, 1, true, false);  
     //display.DrawRect(64,128, 5, 1, true, false);  
...
fillRectangle(ballX, ballY, 2, 2, 1);
 //  hw.PrintLine("draw ball ");
     // display.DrawRect(ballX,ballY, 2, 2, true, true);
     // display.DrawCircle(ballX, ballY, 4, true);
